const eventType = (types) => {
  ...
  <label class="event__type-label  event__type-label--${type}" for="event-type-${type[0]+type.slice(1)}-1">Taxi</label>
</div>`)
}

const eventPhotos = (photos) => {
  return photos.map(photo => 
    `<img class="event__photo" src=${photo.src} alt="Event photo">`).join('')
}
    ...
    <form class="event event--edit" action="#" method="post">
      ...
      <section class="event__details">
        <section class="event__section  event__section--offers">
          ...
          ${eventOffer(point.offers.offers)}
          </div>
        </section>
